...
A `WantsFetch` batch SHALL impose no ordering on its handles: the consumer MAY fetch them in any order and concurrently, and SHALL return results keyed by handle. The engine SHALL match fetched results by handle, not by position, so a consumer servicing the batch across a connection pool is correct.
...
A fetch resolving a placement to a link id another placement of the same collection already carries SHALL give that placement a **minted** link id (pimdir STORAGE §9, `dup:<hint>#<handle>`) and link it, rather than leaving it unlinked. The minted form SHALL be derived from the hint and the placement's own handle alone, so it is deterministic: the same collection re-read from scratch mints the same key, and a rebuild carries it rather than re-deriving it.
...
That load SHALL ask for the key the mint would take as well as for the hint, and a mint landing on a key another row holds SHALL be minted again over it. A source is free to spell its own identity like a minted one, and a mint walking into a held key is the collision the check exists to prevent. Minting again terminates, each round producing a longer key while the held set is finite.

The minted form is one derivation shared by every verb that hands a key out (`PimdirLinkId::minted`), the rebuild and the staged create included, since a form three implementations have to agree on cannot be composed at each call.

#### Scenario: A mint never takes a held key
- GIVEN a collection holding an identity twice, and a third member whose own identity is spelled like the key the second copy would be minted under
- WHEN all three are hydrated
- THEN the third member keeps its own identity and the second copy takes a key no row holds

#### Scenario: The second copy is stored
- GIVEN a collection whose placement already holds a link id
- WHEN a fetch resolves another handle of that collection to the same link id
- THEN that placement is linked under a minted key, with its own body, summary and base
...
### Requirement: A summarising fetch does not lower a hydrated row
Applying a fetch that carries no body SHALL write the level the row's payload supports, `Full` while it holds an object and `Meta` otherwise, rather than the tier the fetch was made at. The level is a claim and the payload is the fact, which is the rule an upgrade already revisits a row by: written the other way round, a row holding a body is demoted by the pass that summarised it, the next full upgrade refetches a body the store holds, and a hub folding the row in lowers the shared level for every source bound to the item.

#### Scenario: A summary refreshed over a cached body
- GIVEN a placement holding a body and no summary
- WHEN it is upgraded to `Meta`
- THEN the summary lands and the placement still reads `Full`
