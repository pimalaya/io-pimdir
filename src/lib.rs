...
//! The [`sql`] module inlines the canonical pimdir schema and statements from
//! the spec's migrations/ and queries/. The [`codec`] module holds the pure,
//! I/O-free encodings between the [`io_replica`] model and the store columns
//! (the flag JSON and the detail-level integer map). The [`client`] module
//! (feature `client`) is [`PimdirStore`], which runs the statements against
//! SQLite (rusqlite) and the blob files, servicing the storage seam.
//!
