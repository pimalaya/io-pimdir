...
/// Runs a `PimdirMutate` against the store to completion, the same
/// load/write pump a client runs, so a test exercises the real staged
/// mutation path end to end.
fn run_mutation(store: &mut PimdirSourceStore, collection: &str, mutation: PimdirMutation) {
    let mut coroutine = PimdirMutate::new(collection.to_string(), mutation);
    let mut arg: Option<PimdirArg> = None;
    loop {
        match coroutine.resume(arg.take()) {
            PimdirCoroutineState::Yielded(PimdirYield::WantsLoad { collection, scope }) => {
                arg = Some(PimdirArg::Load(store.load(&collection, &scope).unwrap()));
            }
            PimdirCoroutineState::Yielded(PimdirYield::WantsWrite(ops)) => {
                store.write(ops).unwrap();
                arg = Some(PimdirArg::Write);
            }
            PimdirCoroutineState::Yielded(other) => panic!("unexpected yield: {other:?}"),
            PimdirCoroutineState::Complete(result) => {
                result.unwrap();
                return;
            }
        }
...
fn meta_placement(handle: &str, link: &str, subject: &str) -> PimdirPlacement {
    let summary = PimdirSummary::Mail(PimdirMailSummary {
...
fn single_source_write_reopen_lookup_and_gc() {
    ...
    store
        .write(vec![
            ...
            PimdirWriteOp::SetCheckpoint {
                collection: inbox(),
                checkpoint: PimdirCheckpoint(vec![1, 2, 3]),
            },
        ])
        .unwrap();
    ...
    drop(store);
    let mut store = PimdirStore::open(dir.path()).unwrap().for_source("local");
    let loaded = store.load(&inbox(), &PimdirLoadScope::All).unwrap();
    ...
    assert_eq!(loaded.checkpoint, Some(PimdirCheckpoint(vec![1, 2, 3])));
...
fn collection_kind_is_declared_and_survives_a_lazy_write() {
    ...
    store
        .write(vec![PimdirWriteOp::SetCheckpoint {
            collection: inbox(),
            checkpoint: PimdirCheckpoint(vec![9]),
        }])
        .unwrap();
    assert_eq!(
        store.collection_kind("INBOX").unwrap().as_deref(),
...
fn two_source_copy_and_delete_propagation() {
    ...
    let right_view = right.load(&inbox(), &PimdirLoadScope::All).unwrap();
    assert_eq!(right_view.placements.len(), 1);
    assert_eq!(right_view.placements[0].status, PimdirStatus::Created);
    assert_eq!(
    ...
    left.write(vec![PimdirWriteOp::DropPlacement {
        collection: inbox(),
        handle: PimdirHandle("L1".into()),
        reason: PimdirDropReason::Deleted,
    }])
    .unwrap();
    ...
    let right_view = right.load(&inbox(), &PimdirLoadScope::All).unwrap();
    assert_eq!(right_view.placements.len(), 1);
    assert_eq!(right_view.placements[0].status, PimdirStatus::Tombstone);
    // left no longer holds it, so it projects nothing rather than a
    // re-copy
    assert!(
        left.load(&inbox(), &PimdirLoadScope::All)
            .unwrap()
            .placements
            .is_empty()
    );
...
fn client_reads_collections_items_page_and_one() {
    ...
    store
        .write(vec![
            ...
            PimdirWriteOp::UpsertPlacement(placement("3", "mid:c", "cafebac0", &[])),
            PimdirWriteOp::UpsertPlacement(placement("1", "mid:a", "cafebabe", &["\\Seen"])),
    ...
    assert_eq!(page1[0].level, PimdirLevel::Full);
    assert_eq!(page1[0].object, Some(PimdirHash("cafebabe".into())));
...
fn client_read_surfaces_level_for_a_meta_only_item() {
    ...
    store
        .write(vec![PimdirWriteOp::UpsertPlacement(meta_placement(
            "1", "mid:a", "hi",
        ))])
        .unwrap();
    ...
    assert_eq!(item.summary.unwrap().title(), "hi");
}
...
fn public_seq_is_message_scoped_global_and_never_reused() {
    ...
    store
        .write(vec![
            PimdirWriteOp::UpsertPlacement(meta_placement("1", "mid:a", "a")),
            PimdirWriteOp::UpsertPlacement(meta_placement("2", "mid:b", "b")),
        ])
        .unwrap();
    ...
    let mut a_in_arch = meta_placement("A1", "mid:a", "a");
    a_in_arch.collection = PimdirCollectionId("Archives".into());
    store
        .write(vec![PimdirWriteOp::UpsertPlacement(a_in_arch)])
        .unwrap();
    assert_eq!(
    ...
    let mut s = meta_placement("S1", "mid:s", "s");
    s.collection = PimdirCollectionId("Sent".into());
    store
        .write(vec![PimdirWriteOp::UpsertPlacement(s)])
        .unwrap();
...
fn client_read_excludes_tombstones() {
    ...
    left.write(vec![
        store_object("cafebabe", b"a"),
        store_object("cafebabf", b"b"),
        PimdirWriteOp::UpsertPlacement(placement("LA", "mid:a", "cafebabe", &[])),
        PimdirWriteOp::UpsertPlacement(placement("LB", "mid:b", "cafebabf", &[])),
    ])
    .unwrap();
    right.load(&inbox(), &PimdirLoadScope::All).unwrap();
    right
        .write(vec![PimdirWriteOp::UpsertPlacement(placement(
            "RA",
            "mid:a",
...
fn a_body_streams_in_and_back_out() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let blobs = io_pimdir::client::blobs::PimdirBlobs::open(dir.path(), PimdirHashAlgo::default());

    // stream a body in, in chunks, committing under its hash
    let mut w = blobs.writer().unwrap();
    w.write_all(b"hello ").unwrap();
    w.write_all(b"world").unwrap();
    let size = w.commit(&PimdirHash("aabbccdd".into())).unwrap();
    assert_eq!(size, 11);
    assert!(
    ...
    let mut file = blobs
        .reader(&PimdirHash("aabbccdd".into()))
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        blobs.get(&PimdirHash("aabbccdd".into())).unwrap().unwrap(),
        b"hello world"
    );
    ...
    assert!(
        blobs
            .reader(&PimdirHash("00000000".into()))
            .unwrap()
            .is_none()
    );
}
...
fn a_byteless_store_object_indexes_a_streamed_blob() {
    ...
    let size = w.commit(&PimdirHash("beef0000".into())).unwrap() as usize;
    ...
    store
        .write(vec![
            PimdirWriteOp::StoreObject {
                object: PimdirObject {
                    hash: PimdirHash("beef0000".into()),
                    size,
                },
                body: None,
            },
            PimdirWriteOp::UpsertPlacement(placement("1", "mid:a", "beef0000", &["\\Seen"])),
        ])
        .unwrap();
    ...
    assert_eq!(
        blobs.get(&PimdirHash("beef0000".into())).unwrap().unwrap(),
        b"streamed-body"
    );
}
...
fn a_shared_blob_survives_until_its_last_referrer_is_purged() {
    ...
    store
        .write(vec![
            ...
            PimdirWriteOp::UpsertPlacement(placement("1", "mid:a", "cafebabe", &["\\Seen"])),
            PimdirWriteOp::UpsertPlacement(placement("2", "mid:b", "cafebabe", &["\\Seen"])),
        ])
        .unwrap();
...
fn a_staged_remove_hides_the_item_but_keeps_it_pending_a_sync() {
    ...
    run_mutation(
        &mut store,
        "INBOX",
        PimdirMutation::Remove(PimdirHandle("1".into())),
    );
...
fn a_staged_move_empties_the_source_and_fills_the_target() {
    ...
    assert_eq!(inbox_proj[0].status, PimdirStatus::Tombstone);
...
fn an_unknown_flag_set_stores_as_null_and_loads_back_unknown() {
    ...
    assert_eq!(placements[1].flags, PimdirFlags::default());
}
...
fn a_base_of_nothing_round_trips_as_a_base() {
    ...
    assert_eq!(loaded[0].status, PimdirStatus::Clean, "not a pending push");
}
